    ...
    switch (color) {
        case 1: // color
            for (k=0;k<15;k++) {
                ...
                for (i=0;i<8;i++) {
                    for (j=0;j<8;j++) {
                        lcd_pix_color(1+j+(renglon-1)*10,1+i+k*8,bitRead(texto[*(msjPtrCpy+k)][i],j),4);
                    }
                }
            }
            break;
        case 0: // black
            for (k=0;k<15;k++) {
                ...
                for (i=0;i<8;i++) {
                    for (j=0;j<8;j++) {
                        lcd_pix(1+j+(renglon-1)*10,1+i+k*8,bitRead(texto[*(msjPtrCpy+k)][i],j),4);
                    }
                }
            }
            break;
